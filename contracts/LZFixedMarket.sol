...
contract FixedPrice is Ownable {
    ...
    constructor(address nft,address usdt){
        nftAddress = nft;
        tokenAddress = usdt;
    }
    ...
    function buy(uint256 nftId,uint256 tokenType) external payable {
        ...
        if(tokenType == USE_MAIN){
            ...
            (bool success,) = address(tokenAddress).call(abi.encodeWithSignature("transferFrom(address,address,uint256)",_msgSender(),FMarkets[nftId].owner,FMarkets[nftId].tokenPrice));
            require(success,"token transfer fail");
            //IERC20(tokenAddress).transferFrom(_msgSender(),FMarkets[nftId].owner, FMarkets[nftId].tokenPrice);
            FMarkets[nftId].buyType = USE_USDT;
        }

        IERC721(nftAddress).safeTransferFrom(address(this),_msgSender(),nftId);
        FMarkets[nftId].state = 3;
        FMarkets[nftId].buyer = _msgSender();
